...
def extract_kfold_times(results_file: Path) -> dict:
    ...
    for entry in results:
        ...
        if "training_time" in entry and entry["training_time"] is not None:
            ...
            total_seconds = entry.get("execution_time", {}).get(
                "seconds", training_only_seconds
            )
            ...
            kfold_candidates.setdefault(k, []).append(
                {
                    ...
                    "vggish_time": vggish_time_seconds / 60
                    if vggish_time_seconds
                    else None,
...
def plot_training_time_vs_kfolds(data: dict, duration: str, output_dir: Path):
    ...
    ax.set_title(
        f"Tiempo de Entrenamiento vs K-Folds\nSegmentos de {duration}", fontsize=14
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Graficar tiempo de entrenamiento vs k-folds"
    )
    parser.add_argument(
        "--duration",
        "-d",
    ...
    output_dir = (
        Path(args.output) if args.output else project_dir / args.duration / "metricas"
    )
    ...
    for k, info in data.items():
        ...
        print(
            f"  K={k:<6} {info['training_time']:>10.2f} min    {vggish_str:<15} {cache_str}"
        )
    print("-" * 50)
