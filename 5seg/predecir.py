...
Uso:
    ...
    python 5seg/predecir.py --evaluar          # Evalúa en conjunto holdout (vida real)
"""
...
def evaluate_holdout_set():
    ...
    print("\nMatrices de Confusión:")
    print("\nPlate Thickness:")
    cm_plate = confusion_matrix(y_true_plate, y_pred_plate)
    print(cm_plate)
    print(f"Clases: {plate_encoder.classes_}")

    print("\nElectrode Type:")
    cm_electrode = confusion_matrix(y_true_electrode, y_pred_electrode)
    print(cm_electrode)
    ...
    inference_result = {
        ...
        "voting_method": "soft",
        "accuracy": {
            "plate_thickness": acc_plate,
            "electrode": acc_electrode,
            "current_type": acc_current,
        },
        "classification_reports": {
            "plate_thickness": classification_report(y_true_plate, y_pred_plate, output_dict=True),
            "electrode": classification_report(y_true_electrode, y_pred_electrode, output_dict=True),
            "current_type": classification_report(y_true_current, y_pred_current, output_dict=True),
        },
        "confusion_matrices": {
...
def predict_single_audio(audio_path):
    ...
    inference_result = {
        ...
        "probabilities": {
            "plate_thickness": dict(zip(plate_encoder.classes_.tolist(), result["probs_plate"].round(4).tolist())),
            "electrode": dict(zip(electrode_encoder.classes_.tolist(), result["probs_electrode"].round(4).tolist())),
            "current_type": dict(zip(current_type_encoder.classes_.tolist(), result["probs_current"].round(4).tolist())),
        },
    }
    save_inference_result(inference_result)
